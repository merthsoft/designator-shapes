...
namespace Merthsoft.DesignatorShapes.Shapes
{
    public static class FreeformLine
    {
        ...
        private static List<IntVec3> _designation = new List<IntVec3>();
        private static List<IntVec2> _path = new List<IntVec2>();
        ...
        private static IntVec2 Min(IntVec2 a, IntVec2 b)
        {
            return new IntVec2(Math.Min(a.x, b.x), Math.Min(a.z, b.z));
        }

        private static IntVec2 Max(IntVec2 a, IntVec2 b)
        {
            return new IntVec2(Math.Max(a.x, b.x), Math.Max(a.z, b.z));
        }
        ...
        private static bool GetPixel(int x, int z)
        {
            if (x < 0 || z < 0 || x >= _oldThickness || z >= _oldThickness) return false;
            return _drawMask[x, z];
        }
    }
}
